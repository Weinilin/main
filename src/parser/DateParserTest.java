...
public class DateParserTest {
	...
	public void test() {
		...
		// test date keyword 3 without year, the case sensitivity, space
		// sensitivity
		dates.clear();
		...
		// sensitivity
		dates.clear();
		dates.add("13/03/2016");
		assertEquals(dates, DateParser.extractDate("mds sale 13 MARCH2016"));
		...
		assertEquals(dates,
				DateParser.extractDate("mds sale from tmr to next year"));
		...
		dates.clear();
		dates.add("23/03/2015");
		assertEquals(dates, DateParser.extractDate("mds sale after today")); // set
		// the
		// date
		// to
		// today
